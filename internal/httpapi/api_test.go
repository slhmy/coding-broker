...
func TestFrontendFallbackServesNonAPIGetRoutes(t *testing.T) {
	handler, st, cleanup := testAPIWithFrontend(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.Contains(rec.Body.String(), "<!doctype html>") {
		t.Fatalf("expected frontend body, got %s", rec.Body.String())
	}
}

func TestFrontendFallbackDoesNotHandleUnknownAPIRoutes(t *testing.T) {
	handler, st, cleanup := testAPIWithFrontend(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("frontend handler should not serve API routes")
	}))
	defer cleanup()
	_ = st
...
func TestDeleteProjectRemovesWorkspaceRecords(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()

	sessionID := createTestSession(t, handler)

	req := httptest.NewRequest(http.MethodDelete, "/api/projects/sample-app", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
	}
	if _, err := st.GetProject(context.Background(), "sample-app"); !errors.Is(err, store.ErrNotFound) {
...
func TestDeleteProjectRejectsActiveSessions(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()
	...
	if err := st.UpdateSessionStatus(context.Background(), sessionID, model.SessionStatusRunning, session.WorktreeID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := st.GetProject(context.Background(), "sample-app"); err != nil {
		t.Fatalf("expected project to remain, got %v", err)
	}
}
...
func TestPushProjectReturnsPushMessage(t *testing.T) {
	handler, cleanup := testAPI(t)
	defer cleanup()
	...
	runGit(t, repoPath, "remote", "add", "origin", remotePath)
	runGit(t, repoPath, "push", "-u", "origin", "main")
	runGit(t, repoPath, "commit", "--allow-empty", "-m", "local change")
	...
	req = httptest.NewRequest(http.MethodPost, "/api/projects/sample-app/git/push", nil)
	rec = httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("expected push status %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
	}
	...
	if response.PushMessage == nil || strings.TrimSpace(*response.PushMessage) == "" {
		t.Fatalf("unexpected push message: %#v", response.PushMessage)
	}
	if response.LastPushedAt == nil {
		t.Fatal("expected lastPushedAt to be set")
	}
}

func TestGetProjectDetailCountsLocalCommitsAheadOfOrigin(t *testing.T) {
	handler, cleanup := testAPI(t)
	defer cleanup()
	...
	if response.Git.Ahead != 1 || response.Git.Behind != 0 {
		t.Fatalf("expected ahead/behind 1/0, got %d/%d", response.Git.Ahead, response.Git.Behind)
	}
}

func TestPushProjectReturnsNotFoundForMissingProject(t *testing.T) {
	handler, cleanup := testAPI(t)
	defer cleanup()

	req := httptest.NewRequest(http.MethodPost, "/api/projects/missing/git/push", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("expected status %d, got %d: %s", http.StatusNotFound, rec.Code, rec.Body.String())
...
func TestDeleteSessionRemovesSession(t *testing.T) {
	...
	createSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app","prompt":"test session","useCurrentBranch":false}`)
...
func TestDeleteSessionCancelsAgentAfterStoreDelete(t *testing.T) {
	...
	createSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app","prompt":"delete cancel agent","useCurrentBranch":false}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions", createSessionBody)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if runner.cancelSessionID != session.ID {
		t.Fatalf("expected runner cancel for %q, got %#v", session.ID, runner)
	}
	if _, err := st.GetSession(context.Background(), session.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("expected store session to be deleted before cancel assertion, got %v", err)
	}
}

func TestDeleteSessionRejectsCurrentBranchSession(t *testing.T) {
	handler, cleanup := testAPI(t)
	defer cleanup()

	createProject(t, handler)
	createSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app","useCurrentBranch":true}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions", createSessionBody)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if session.Deletable {
		t.Fatalf("expected current branch session to be non-deletable, got %#v", session)
	}
...
func TestListSessionsReturnsSessionRecords(t *testing.T) {
	...
	if !session.Deletable {
		t.Fatalf("expected worktree session to be deletable, got %#v", session)
	}
...
func TestListSessionsFiltersByProject(t *testing.T) {
	...
	sampleSessionID := createTestSession(t, handler)
	otherRepoPath := createNamedProject(t, handler, "Other App")
	if strings.TrimSpace(otherRepoPath) == "" {
		t.Fatal("expected other repo path")
	}
	otherSessionID := createSessionForProject(t, handler, "other-app")

	req := httptest.NewRequest(http.MethodGet, "/api/sessions?project=sample-app", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("expected status %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
	...
	var sessions []sessionRecordResponse
	if err := json.NewDecoder(rec.Body).Decode(&sessions); err != nil {
		t.Fatal(err)
	}
	if len(sessions) != 1 || sessions[0].ID != sampleSessionID {
		t.Fatalf("expected only sample session %q, got %#v", sampleSessionID, sessions)
	}
	if sessions[0].ID == otherSessionID {
		t.Fatalf("expected other session %q to be filtered out", otherSessionID)
	}
}

func TestMarkSessionReadMovesUnreadTerminalSessionToIdleWithoutReordering(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()

	for _, terminalStatus := range []model.SessionStatus{model.SessionStatusDone, model.SessionStatusFailed} {
		sessionID := createTestSession(t, handler)
		sessionBefore, err := st.GetSession(context.Background(), sessionID)
		if err != nil {
			t.Fatal(err)
		}
		if err := st.UpdateSessionStatus(context.Background(), sessionID, terminalStatus, sessionBefore.WorktreeID); err != nil {
			t.Fatal(err)
		}
		sessionBefore, err = st.GetSession(context.Background(), sessionID)
		if err != nil {
		...
		req := httptest.NewRequest(http.MethodPost, "/api/sessions/"+sessionID+"/read", nil)
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, req)
		if rec.Code != http.StatusOK {
			t.Fatalf("expected status %d, got %d: %s", http.StatusOK, rec.Code, rec.Body.String())
		...
		sessionAfter, err := st.GetSession(context.Background(), sessionID)
		if err != nil {
			t.Fatal(err)
		}
		if sessionAfter.Status != model.SessionStatusIdle {
			t.Fatalf("expected %s session to be idle, got %q", terminalStatus, sessionAfter.Status)
		}
		if !sessionAfter.UpdatedAt.Equal(sessionBefore.UpdatedAt) {
			t.Fatalf("expected updatedAt to stay %s, got %s", sessionBefore.UpdatedAt, sessionAfter.UpdatedAt)
		}
	}
}
...
func TestGetSessionReturnsWorktreeGitStatus(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()

	repoPath := createProject(t, handler)
	worktreePath := filepath.Join(t.TempDir(), "session-clean-worktree")
	runGit(t, repoPath, "worktree", "add", "-b", "feature/session-clean", worktreePath, "main")
	if err := os.WriteFile(filepath.Join(repoPath, "notes.txt"), []byte("dirty main"), 0o644); err != nil {
		t.Fatal(err)
	}

	sessionID := createSessionForProject(t, handler, "sample-app")
	worktree, err := st.SaveWorktree(context.Background(), model.Worktree{
		ID:          "wt-session-clean",
		SessionID:   sessionID,
		ProjectSlug: "sample-app",
		Path:        worktreePath,
		Branch:      "feature/session-clean",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if detail.Worktree.Git.DirtyFiles != 0 {
		t.Fatalf("expected clean worktree git status, got %#v", detail.Worktree.Git)
	}
	if detail.Worktree.Git.Branch != "feature/session-clean" {
		t.Fatalf("expected worktree branch status, got %#v", detail.Worktree.Git)
	}
}
...
func TestDeleteSessionRemovesReferencedWorktree(t *testing.T) {
	...
	createProject(t, handler)
	createSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app","prompt":"delete worktree session","useCurrentBranch":false}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions", createSessionBody)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if session.WorktreeID == "" {
		t.Fatalf("expected worktree session, got %#v", session)
	}
	worktree, err := st.GetWorktree(context.Background(), session.WorktreeID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(worktree.Path); err != nil {
		t.Fatalf("expected worktree path to exist before delete, got %v", err)
	}
	...
	if _, err := st.GetSession(context.Background(), session.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("expected referenced session to be deleted, got %v", err)
	}
	...
	if _, err := os.Stat(worktree.Path); !os.IsNotExist(err) {
		t.Fatalf("expected worktree path to be removed, got %v", err)
	}
}
...
func TestCancelSessionMarksSessionIdle(t *testing.T) {
	...
	if len(detail.TimelineEvents) == 0 || detail.TimelineEvents[len(detail.TimelineEvents)-1].Title != "Agent canceled" {
		t.Fatalf("expected cancel event, got %#v", detail.TimelineEvents)
	}
}
...
func TestAddMessageQueuesPromptForActiveSession(t *testing.T) {
	...
	createProject(t, handler)
	createSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions", createSessionBody)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if rec.Code != http.StatusAccepted {
		t.Fatalf("expected first prompt status %d, got %d: %s", http.StatusAccepted, rec.Code, rec.Body.String())
	}
	<-agent.respondStarted

	req = httptest.NewRequest(http.MethodPost, "/api/sessions/"+sessionID+"/messages", bytes.NewBufferString(`{"content":"queued"}`))
	req.Header.Set("Content-Type", "application/json")
	rec = httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusAccepted {
		t.Fatalf("expected queued prompt status %d, got %d: %s", http.StatusAccepted, rec.Code, rec.Body.String())
	}
	var accepted []model.Message
	if err := json.NewDecoder(rec.Body).Decode(&accepted); err != nil {
		t.Fatal(err)
	}
	if len(accepted) != 1 || accepted[0].Content != "queued" {
		t.Fatalf("unexpected queued response: %#v", accepted)
	}
	session, err := st.GetSession(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
	...
	if session.Status != model.SessionStatusRunning {
		t.Fatalf("expected queued prompt to keep session running, got %q", session.Status)
	}
	messages, err := st.ListMessages(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
	}
	if !hasQueuedUserMessage(messages) {
		t.Fatalf("expected queued user message, got %#v", messages)
	}
	detail, err := st.GetSessionDetail(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
	}
	if len(detail.TimelineEvents) == 0 || detail.TimelineEvents[len(detail.TimelineEvents)-1].Title != "Prompt queued" {
		t.Fatalf("expected queue event, got %#v", detail.TimelineEvents)
	}
	if detail.TimelineEvents[len(detail.TimelineEvents)-1].Summary != "1 pending prompt(s) will run after the current response." {
		t.Fatalf("expected one pending prompt, got %#v", detail.TimelineEvents[len(detail.TimelineEvents)-1])
	}

	req = httptest.NewRequest(http.MethodPost, "/api/sessions/"+sessionID+"/messages", bytes.NewBufferString(`{"content":"queued again"}`))
	req.Header.Set("Content-Type", "application/json")
	rec = httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusAccepted {
		t.Fatalf("expected second queued prompt status %d, got %d: %s", http.StatusAccepted, rec.Code, rec.Body.String())
	}
	detail, err = st.GetSessionDetail(context.Background(), sessionID)
	if err != nil {
		t.Fatal(err)
	}
	if len(detail.TimelineEvents) == 0 || detail.TimelineEvents[len(detail.TimelineEvents)-1].Summary != "2 pending prompt(s) will run after the current response." {
		t.Fatalf("expected two pending prompts, got %#v", detail.TimelineEvents)
	}
	close(agent.releaseRespond)
	waitForSessionStatus(t, st, sessionID, model.SessionStatusDone)
	if agent.respondUserMessages[len(agent.respondUserMessages)-1] != "queued again" {
		t.Fatalf("expected agent to process queued prompt, got %#v", agent.respondUserMessages)
	}
}
...
func TestAddMessageSavesPermissionProgressEvent(t *testing.T) {
	...
	for _, event := range detail.TimelineEvents {
		if event.Kind == "permission_request" {
			var payload struct {
				PermissionID string `json:"permissionId"`
			}
			if err := json.Unmarshal(event.Payload, &payload); err == nil && payload.PermissionID == permissions[0].ID {
	...
	if !foundApprovalEvent {
		t.Fatalf("expected progress event with approval ID %q, got %#v", permissions[0].ID, detail.TimelineEvents)
	}
}

func TestAddMessageSavesPatchProgressAsFileChangeEvent(t *testing.T) {
	agent := &fakeAgentRunner{
		progressEvents: []agent.ProgressEvent{{
			Kind:      "tool_use",
			ToolName:  "Patch",
			ToolInput: `[{"path":"frontend/src/pages/session.tsx","type":"modified"}]`,
			Content:   "Using Patch",
		}},
	}
	handler, st, cleanup := testAPIWithStoreAndAgent(t, agent)
	...
	event := findTimelineEvent(detail.TimelineEvents, "file_change")
	if event == nil {
		t.Fatalf("expected file_change event, got %#v", detail.TimelineEvents)
	}
	var payload struct {
		Files []struct {
			Path      string `json:"path"`
	...
	if len(payload.Files) != 1 || payload.Files[0].Path != "frontend/src/pages/session.tsx" || payload.Files[0].Operation != "modified" {
		t.Fatalf("unexpected file_change payload: %#v", payload)
	}
	if !strings.Contains(payload.Patch, "frontend/src/pages/session.tsx") {
		t.Fatalf("expected payload patch to include path, got %#v", payload)
	}
}

func TestAddMessageSavesDiffToolResultAsFileChangeEvent(t *testing.T) {
	diff := `diff --git a/frontend/src/pages/session.tsx b/frontend/src/pages/session.tsx
...
+new`
	...
	if len(payload.Files) != 1 || payload.Files[0].Path != "frontend/src/pages/session.tsx" {
		t.Fatalf("unexpected diff file payload: %#v", payload)
	}
	if payload.Additions != 1 || payload.Deletions != 1 || payload.Files[0].Additions != 1 || payload.Files[0].Deletions != 1 {
		t.Fatalf("unexpected diff counts: %#v", payload)
	}
	if !strings.Contains(payload.Patch, "diff --git") {
		t.Fatalf("expected diff patch, got %#v", payload)
	}
}
...
func TestCreateSessionReturnsSessionRecord(t *testing.T) {
	...
	for _, key := range []string{"id", "title", "status", "mode", "model", "projectSlug", "createdAt", "updatedAt", "deletable"} {
		if _, ok := response[key]; !ok {
...
func TestIntegrateProjectWorktreeUsesTargetSessionWhenProvided(t *testing.T) {
	handler, st, cleanup := testAPIWithStoreAndAgent(t, &fakeAgentRunner{})
	defer cleanup()

	createProject(t, handler)

	worktreeBody := bytes.NewBufferString(`{"name":"source-worktree","branch":"feature/source-worktree"}`)
	worktreeReq := httptest.NewRequest(http.MethodPost, "/api/projects/sample-app/git/worktrees", worktreeBody)
	...
	if worktreeRec.Code != http.StatusCreated {
		t.Fatalf("expected status %d, got %d: %s", http.StatusCreated, worktreeRec.Code, worktreeRec.Body.String())
	}
	var worktree struct {
		ID string `json:"id"`
	}
	if err := json.NewDecoder(worktreeRec.Body).Decode(&worktree); err != nil {
		t.Fatal(err)
	}

	sourceSessionBody := bytes.NewBufferString(`{"projectSlug":"sample-app","worktreeId":` + quoteJSON(worktree.ID) + `,"prompt":"source session"}`)
	sourceSessionReq := httptest.NewRequest(http.MethodPost, "/api/sessions", sourceSessionBody)
	sourceSessionReq.Header.Set("Content-Type", "application/json")
	sourceSessionRec := httptest.NewRecorder()
	handler.ServeHTTP(sourceSessionRec, sourceSessionReq)
	if sourceSessionRec.Code != http.StatusCreated {
		t.Fatalf("expected session status %d, got %d: %s", http.StatusCreated, sourceSessionRec.Code, sourceSessionRec.Body.String())
	}
	var sourceSession struct {
		ID string `json:"id"`
	}
	if err := json.NewDecoder(sourceSessionRec.Body).Decode(&sourceSession); err != nil {
		t.Fatal(err)
	}

	targetSessionID := createTestSession(t, handler)

	integrateReq := httptest.NewRequest(http.MethodPost, "/api/projects/sample-app/git/worktrees/"+worktree.ID+"/integrate", bytes.NewBufferString(`{"strategy":"merge","targetSessionId":`+quoteJSON(targetSessionID)+`}`))
	integrateReq.Header.Set("Content-Type", "application/json")
	integrateRec := httptest.NewRecorder()
	handler.ServeHTTP(integrateRec, integrateReq)
	if integrateRec.Code != http.StatusAccepted {
		t.Fatalf("expected accepted status %d, got %d: %s", http.StatusAccepted, integrateRec.Code, integrateRec.Body.String())
	}

	messages, err := st.ListMessages(context.Background(), targetSessionID)
	if err != nil {
		t.Fatal(err)
	}
	if len(messages) == 0 || !strings.Contains(messages[len(messages)-1].Content, "using git merge") {
		t.Fatalf("expected target session merge prompt, got %#v", messages)
	}
	if messages[len(messages)-1].Mode != model.SessionModeAct {
		t.Fatalf("expected target session merge prompt to use act mode, got %q", messages[len(messages)-1].Mode)
	}
	targetSession, err := st.GetSession(context.Background(), targetSessionID)
	if err != nil {
		t.Fatal(err)
	}
	if targetSession.Mode != model.SessionModeAct {
		t.Fatalf("expected target session mode to switch to act, got %q", targetSession.Mode)
	}
}
...
func TestDeleteProjectWorktreeAllowsCompletedWorktreeWithResult(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()

	createProject(t, handler)
	req := httptest.NewRequest(http.MethodPost, "/api/projects/sample-app/git/worktrees", bytes.NewBufferString(`{"name":"completed-worktree","branch":"feature/completed-worktree"}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusCreated {
	...
	worktree, err := st.GetWorktree(context.Background(), created.ID)
	if err != nil {
		t.Fatal(err)
	}
	session, err := st.GetSession(context.Background(), worktree.SessionID)
	if err != nil {
		t.Fatal(err)
	}
	if err := st.UpdateSessionStatus(context.Background(), session.ID, model.SessionStatusDone, worktree.ID); err != nil {
		t.Fatal(err)
	}
	if err := st.UpdateWorktreeResult(context.Background(), worktree.ID, "abc123", true); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := st.GetWorktree(context.Background(), worktree.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("expected worktree to be deleted, got %v", err)
	}
}

func TestListProjectWorktreesKeepsCompletedWorktreeReady(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()

	createProject(t, handler)
	worktree, err := st.SaveWorktree(context.Background(), model.Worktree{
		ID:          "wt-completed-ready",
		ProjectSlug: "sample-app",
		Path:        filepath.Join(t.TempDir(), "completed-ready"),
		Branch:      "feature/completed-ready",
		CommitSHA:   "abc123",
		Pushed:      true,
	})
	if err != nil {
	...
	for _, candidate := range worktrees {
		if candidate.ID == worktree.ID && candidate.Status != "ready" {
			t.Fatalf("expected completed worktree to remain ready, got %#v", candidate)
		}
	}
}

func TestListProjectWorktreesMarksRunningSessionWorktreeActive(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()
...
func TestGetProjectDetailMarksRunningSessionWorktreeActive(t *testing.T) {
	handler, st, cleanup := testAPIWithStore(t)
	defer cleanup()
...
func createTestSession(t *testing.T, handler http.Handler) string {
	...
	createProject(t, handler)
	return createSessionForProject(t, handler, "sample-app")
}

func createSessionForProject(t *testing.T, handler http.Handler, projectSlug string) string {
	t.Helper()

	createSessionBody := bytes.NewBufferString(`{"projectSlug":` + quoteJSON(projectSlug) + `,"prompt":"test session"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions", createSessionBody)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func testAPIWithFrontend(t *testing.T, frontend http.Handler) (http.Handler, *store.Store, func()) {
	t.Helper()
...
func (f *fakeAgentRunner) Respond(ctx context.Context, detail model.SessionDetail, userMessage string, onProgress func(agent.ProgressEvent)) (agent.RespondResult, error) {
	...
	f.respondUserMessages = append(f.respondUserMessages, userMessage)
...
func findTimelineEvent(events []model.TimelineEvent, kind string) *model.TimelineEvent {
	for i := range events {
