...
type Config struct {
	...
	CodexCommand             string
	CodexArgs                []string
	CodexSessionEnv          []string
	CodexSessionEnvAllowlist []string
	DefaultModel             string
...
func Load() (Config, error) {
	...
	v.SetDefault("http.addr", "0.0.0.0:8787")
	...
	v.SetDefault("codex.session_env", []string{})
	v.SetDefault("codex.session_env_allowlist", []string{
		"HTTP_PROXY",
		"HTTPS_PROXY",
		"ALL_PROXY",
		"NO_PROXY",
	...
	return Config{
		...
		CodexArgs:                codexArgs,
		CodexSessionEnv:          normalizeEnvPairs(v.GetStringSlice("codex.session_env")),
		CodexSessionEnvAllowlist: normalizeStringList(v.GetStringSlice("codex.session_env_allowlist")),
...
func normalizeEnvPairs(values []string) []string {
	seen := make(map[string]bool)
	normalized := make([]string, 0, len(values))
	for _, value := range values {
		value = strings.TrimSpace(value)
		...
		seen[key] = true
		normalized = append(normalized, key+"="+val)
	}
	return normalized
}
