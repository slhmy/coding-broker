	...
	for {
		...
		case event, ok := <-session.Events():
			...
			case core.EventToolUse:
				simpleProgress(onProgress, "tool_use", formatToolUse(event))
			case core.EventToolResult:
				simpleProgress(onProgress, "tool_result", formatToolResult(event))
...
func (r *Runner) codexSession(ctx context.Context, detail model.SessionDetail) (core.AgentSession, error) {
	...
	cliPath := strings.TrimSpace(strings.Join(append([]string{r.cfg.CodexCommand}, r.cfg.CodexArgs...), " "))
	opts := map[string]any{
		"work_dir": workDir,
