...
- Codex sessions can inherit environment in two ways: `codex.session_env` lets you set explicit `KEY=VALUE` entries, and `codex.session_env_allowlist` forwards matching values from the broker process. Common proxy and CA vars like `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`, `SSL_CERT_FILE`, and `NODE_EXTRA_CA_CERTS` are included by default. This only affects the Codex subprocess launched by Coding Broker; it does not change the sandbox policy of the current Codex session.
