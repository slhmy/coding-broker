...
function sortSummaries(input: SessionSummary[]) {
  return [...input].sort((left, right) => {
    const updatedAtDifference =
      (parseDateTimeMillis(right.updatedAt) ?? 0) - (parseDateTimeMillis(left.updatedAt) ?? 0)
...
function hasPendingPermission(session: Session) {
  return session.permissions.some((permission) => permission.status === "pending")
}
...
async function pollActiveSessions() {
  ...
  await Promise.allSettled(activeSessionIds.map((sessionId) => refreshSession(sessionId)))
}
...
export async function refreshSession(sessionId: string) {
  ...
  const request = api.session(sessionId)
    .then((session) => {
