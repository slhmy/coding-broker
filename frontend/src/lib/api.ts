...
export const api = {
  config: async () =>
    normalizeAppConfig(await requestJson<Partial<AppConfig>>("/api/config")),
  ...
  projects: async () =>
    sortProjectsByStoreOrder(
      normalizeArray(
        await requestJson<ProjectDetail[] | null>("/api/projects")
  ...
  deleteProject: (projectSlug: string) =>
    requestJson<{ ok: true }>(`/api/projects/${projectSlug}`, {
      method: "DELETE",
    }),
  project: async (projectSlug: string) =>
    normalizeProjectDetail(
      await requestJson<ProjectDetail>(`/api/projects/${projectSlug}`)
    ),
  sessions: async (input?: { projectSlug?: string | null }) => {
    const params = new URLSearchParams()
    if (input?.projectSlug) {
      params.set("project", input.projectSlug)
    }
    const query = params.toString()
    return sortSessionsByStoreOrder(
      normalizeArray(
        await requestJson<SessionRecord[] | null>(
  ...
  updateMode: (sessionId: string, mode: SessionMode) =>
    requestJson<SessionRecord>(`/api/sessions/${sessionId}/mode`, {
      method: "PATCH",
      body: JSON.stringify({ mode }),
    }),
  updateModel: (sessionId: string, model: string) =>
    requestJson<SessionRecord>(`/api/sessions/${sessionId}/model`, {
      method: "PATCH",
      body: JSON.stringify({ model }),
    }),
  markSessionRead: (sessionId: string) =>
    requestJson<SessionRecord>(`/api/sessions/${sessionId}/read`, {
      method: "POST",
    }),
  ...
  clearSessionContext: (sessionId: string) =>
    requestJson<BackendSessionDetail>(
      `/api/sessions/${sessionId}/context/clear`,
      {
        method: "POST",
      }
  ...
  pushMain: (projectSlug: string) =>
    requestJson<GitStatus>(`/api/projects/${projectSlug}/git/push`, {
      method: "POST",
    }),
  worktrees: async (projectSlug: string) =>
    sortWorktreesByStoreOrder(
      normalizeArray(
        await requestJson<Worktree[] | null>(
          `/api/projects/${projectSlug}/git/worktrees`
        )
      )
    ),
  ...
  pullWorktree: (projectSlug: string, worktreeId: string) =>
    requestJson<GitStatus>(
      `/api/projects/${projectSlug}/git/worktrees/${worktreeId}/pull`,
      { method: "POST" }
    ),
  pushWorktree: (projectSlug: string, worktreeId: string) =>
    requestJson<GitStatus>(
      `/api/projects/${projectSlug}/git/worktrees/${worktreeId}/push`,
      { method: "POST" }
    ),
  integrateWorktree: (
    projectSlug: string,
    worktreeId: string,
    input: IntegrateWorktreeInput
  ) =>
    requestJson<{ session: SessionRecord; messages: Message[] }>(
      `/api/projects/${projectSlug}/git/worktrees/${worktreeId}/integrate`,
      {
        method: "POST",
        body: JSON.stringify(input),
      }
...
function normalizeAppConfig(config: Partial<AppConfig>): AppConfig {
  ...
  return {
    ...
    worktreeRoot:
      typeof config.worktreeRoot === "string" ? config.worktreeRoot : "",
  }
}
...
function normalizeSessionDetail(detail: BackendSessionDetail): Session {
  ...
  const timelineEvents = sortCreatedAtThenId(
    normalizeArray(detail.timelineEvents)
  )
